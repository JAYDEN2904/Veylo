/** Client mirror of edge itemMetadata — gender + occasion derivation. */
...
export function deriveGenderAffinity(options: {
  ...
  const blob = blobFromParts([options.category, options.subCategory, ...(options.styleTags ?? [])]);
...
export function deriveOccasionTags(options: {
  ...
  for (const rule of OCCASION_RULES) {
    if (rule.tokens.some((t) => blob.includes(t))) found.add(rule.tag);
  }
